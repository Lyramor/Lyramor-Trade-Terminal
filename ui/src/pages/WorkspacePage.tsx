      ...
      <div className="flex items-center justify-between px-3 py-1.5 border-b border-border bg-bg-secondary/30 shrink-0">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-2 py-1 rounded-md text-[11px] text-accent hover:bg-accent/10 transition-colors"
            title="Start a new session in this workspace (⌘T)"
          >
            <MessageSquare size={13} strokeWidth={2.25} aria-hidden="true" />
            New chat
          </button>
          {/* The escape hatch: the raw PTY is still one click away for anyone
            * who wants the real CLI (and it's the only surface for agents
            * without a chat mode). */}
          <button
            type="button"
            onClick={() => void ctx.spawn(wsId, { transport: 'pty' })}
            ...
            title="Spawn a raw terminal session (the CLI TUI) in this workspace"
          >
