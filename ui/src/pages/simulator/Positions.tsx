...
export function Positions({ state }: { state: SimulatorState }) {
  ...
  return (
    ...
    >
      {/* Lima kolom angka. Kolom PnL yang paling kanan justru yang paling
          dicari, jadi wajib bisa dijangkau di layar sempit. */}
      {state.positions.length === 0 ? (
        <p className="text-xs text-text-muted">No positions.</p>
      ) : (
        <TableScroll label="Simulator positions" bordered={false}>
