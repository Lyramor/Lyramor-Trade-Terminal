...
export function createReferenceData(deps: ReferenceDataDeps): ReferenceDataService {
  ...
  // Macro board is FRESH-FIRST: pull live from the keyless `federal_reserve`
  // provider so the board reflects the latest FRED release, and fall back to
  // the hosted hub snapshot only when the direct fetch is unreachable (no
  // network / DNS blocked). The hub snapshot can lag the source by hours, so
  // preferring it (the old behaviour) made the board look stale even when FRED
  // itself was current.
  const macro = cachedBoard(TTL.macro, async () => {
    try {
      return markLocal(await fetchMacroBoard(deps.economyClient))
    } catch (err) {
      const hub = await viaHub<MacroBoard>('macro')
      if (hub) return hub
      throw err
    }
  })
