...
export const opencodeAdapter: CliAdapter = {
  ...
  capabilities: {
    ...
    headless: true,
    // Per-turn chat (NOT claude's persistent stream-json): each user message is a
    // fresh `opencode run --format json` that resumes the prior turn's `ses_…`.
    // See composeChatTurn / createChatNormalizer.
    chat: true,
  },
  ...
  // Per-turn chat: `opencode run --format json` reads the user message from
  // STDIN when no positional `message` is given (verified 1.16.x) — so the
  // message never rides the argv, closing the win32 `.cmd`-shim → cmd.exe
  // injection surface (deliver:'stdin'). Resume carries context across turns via
  // `--session <ses_…>` (verified: a second `run --session <id>` recalled a fact
  // set in the first). MCP + provider ride OPENCODE_CONFIG_CONTENT / opencode.json
  // (composeEnv / writeAiConfig), same as interactive, so `run` keeps its tools.
  composeChatTurn(ctx: ChatTurnContext): ChatTurnPlan {
    const command = ['opencode', 'run', '--format', 'json'];
    if (ctx.resumeSessionId) command.push('--session', ctx.resumeSessionId);
    return { command, deliver: 'stdin' };
  },

  // Normalize `opencode run --format json` events onto the Claude envelope.
  // Verified shapes (1.16.x): each line is `{type, sessionID, part}` where
  //   part.type==='text'  → assistant text (part.text; finalized once
  //                          part.time.end is set — emit once, keyed by part.id
  //                          so streaming partials don't duplicate the bubble)
  //   part.type==='tool'  → a tool_use (part.tool = name, part.state.input =
  //                          args; emit once per part.id at running/completed)
  // step_start / step_finish carry no user-visible content → dropped. The
  // closing `result` is synthesized by the per-turn session on process exit.
  createChatNormalizer(): (line: string) => ChatEvent[] {
    ...
    return (line: string): ChatEvent[] => {
      ...
      const part = asRecord(evt['part']);
      if (!part) return [];
      const ptype = part['type'];
      const id = typeof part['id'] === 'string' ? (part['id'] as string) : '';
      if (ptype === 'text') {
        const text = typeof part['text'] === 'string' ? (part['text'] as string) : '';
        const time = asRecord(part['time']);
        const finalized = time ? time['end'] != null : false;
        if (!finalized || !text || (id && emittedText.has(id))) return [];
      ...
      if (ptype === 'tool') {
        ...
        const status = state ? state['status'] : undefined;
        if (status !== 'running' && status !== 'completed' && status !== 'error') return [];
        if (id) emittedTool.add(id);
        const name = typeof part['tool'] === 'string' ? (part['tool'] as string) : 'tool';
