...
function asPiRecord(v: unknown): Record<string, unknown> | null {
...
export const piAdapter: CliAdapter = {
  ...
  capabilities: {
    ...
    headless: true,
    // Per-turn chat: each user message is a fresh `pi -p --mode json` bound to a
    // launcher-assigned `--session-id` (create-or-reopen), so context carries
    // across turns without harvesting an id. See composeChatTurn / createChatNormalizer.
    chat: true,
  },
  ...
  // Per-turn chat. pi installs as an npm `.cmd` shim on win32 → its spawn routes
  // through `cmd.exe /d /c` (win-command.ts), so the user message MUST go over
  // stdin, never the argv (deliver:'stdin'). `--session-id <id>` is
  // create-or-reopen (assignsSessionId), so the launcher passes the SAME id every
  // turn — pi creates it on turn 1, reopens it after — giving deterministic
  // multi-turn context with no id harvesting.
  //
  // UNVERIFIED (pi not installed on this host): (1) that `pi -p --mode json`
  // reads the prompt from stdin when no positional is given, and (2) the
  // `--mode json` event field names in createChatNormalizer. If pi does NOT read
  // stdin, the win32 path will hang the turn — confirm with `pi -p --help` /
  // `echo hi | pi -p --mode json` before trusting the win32 chat path. On
  // mac/linux pi resolves by bare name (no cmd.exe), so stdin is still safe.
  composeChatTurn(ctx: ChatTurnContext): ChatTurnPlan {
    return {
      command: ['pi', '-p', '--mode', 'json', '--session-id', ctx.assignedSessionId],
      deliver: 'stdin',
    };
  },

  // Normalize `pi -p --mode json` events onto the Claude envelope. UNVERIFIED
  // (pi not installed here). line 1 is `{type:'session', id}` (handled by
  // extractHeadlessSessionId, not here). Best-effort mapping of the turn events:
  //   {type:'assistant', text|content}                 → assistant text
  //   {type:'text'|'message', text}                    → assistant text
  //   {type:'tool_use'|'tool', name|tool, input|args}  → tool_use
  // The closing `result` is synthesized by the per-turn session on exit. Confirm
  // the real event grammar against `pi -p --mode json` output.
  createChatNormalizer(): (line: string) => ChatEvent[] {
    return (line: string): ChatEvent[] => {
      ...
      const pickText = (): string => {
        if (typeof evt['text'] === 'string') return evt['text'] as string;
        const content = evt['content'];
        if (typeof content === 'string') return content;
        if (Array.isArray(content)) {
          return content
            .map((b) => {
              const r = asPiRecord(b);
      ...
      if (t === 'assistant' || t === 'message' || t === 'text') {
        const text = pickText();
        if (!text) return [];
        ...
        const name =
          typeof evt['name'] === 'string'
            ...
            : typeof evt['tool'] === 'string'
              ? (evt['tool'] as string)
              : 'tool';
        const input = evt['input'] ?? evt['args'] ?? evt['arguments'] ?? {};
        return [
          {
            type: 'assistant',
