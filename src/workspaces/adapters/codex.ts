...
function asCodexRecord(v: unknown): Record<string, unknown> | null {
...
export const codexAdapter: CliAdapter = {
  ...
  capabilities: {
    ...
    headless: true,
    // Per-turn chat: each user message is a fresh `codex exec --json` that
    // resumes the prior turn's thread id. See composeChatTurn / createChatNormalizer.
    chat: true,
  },
  ...
  // Per-turn chat. codex ships a NATIVE exe on every platform (codex.exe on
  // win32), so it never routes through cmd.exe — the user message rides the argv
  // safely (deliver:'arg'; the trailing `--` end-of-options terminator guards a
  // `-`-leading message). Same headless CLI-mode `-c` globals as
  // composeHeadlessCommand (approval/sandbox/network) so `alice*` works without a
  // human approver.
  //
  // UNVERIFIED (codex not installed on this host): the resume subcommand shape
  // `codex exec resume <thread_id>` and that it accepts `--json`/`--`. Confirm
  // with `codex exec resume --help` before trusting multi-turn context carry;
  // if wrong, the second turn starts a fresh thread (context loss), not a crash.
  composeChatTurn(ctx: ChatTurnContext): ChatTurnPlan {
    ...
    const exec = ctx.resumeSessionId
      ? ['exec', 'resume', ctx.resumeSessionId, '--json', '--']
      : ['exec', '--json', '--'];
    return { command: [...globals, ...exec], deliver: 'arg' };
  },

  // Normalize `codex exec --json` events onto the Claude envelope. UNVERIFIED
  // (codex not installed here) — mapped from the documented event grammar:
  //   {type:'item.completed', item:{type:'assistant_message', text}}      → assistant text
  //   {type:'item.*',         item:{type:'command_execution', command}}   → tool_use('shell')
  //   {type:'item.*',         item:{type:'mcp_tool_call', server, tool}}  → tool_use(tool)
  // thread.started / turn.* carry no bubble content. The closing `result` is
  // synthesized by the per-turn session on exit. Assistant text is emitted only
  // on `item.completed` (final), keyed by item.id, so streaming updates don't
  // duplicate the bubble. Verify field names against `codex exec --json` live.
  createChatNormalizer(): (line: string) => ChatEvent[] {
    ...
    return (line: string): ChatEvent[] => {
      ...
      const item = asCodexRecord(evt['item']);
      if (!item) return [];
      const itype = item['type'];
      const id = typeof item['id'] === 'string' ? (item['id'] as string) : '';
      const evtType = evt['type'];
      if (itype === 'assistant_message') {
        if (evtType !== 'item.completed') return [];
        const text = typeof item['text'] === 'string' ? (item['text'] as string) : '';
        if (!text || (id && emittedText.has(id))) return [];
        if (id) emittedText.add(id);
        return [{ type: 'assistant', message: { role: 'assistant', content: [{ type: 'text', text }] } }];
      }
      if (itype === 'command_execution' || itype === 'mcp_tool_call') {
        if (id && emittedTool.has(id)) return [];
        if (id) emittedTool.add(id);
        const name = itype === 'mcp_tool_call'
          ? (typeof item['tool'] === 'string' ? (item['tool'] as string) : 'tool')
          : 'shell';
        const input = item['command'] ?? item['arguments'] ?? item['input'] ?? {};
        return [
          {
            type: 'assistant',
